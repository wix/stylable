...
describe(`features/css-class`, () => {
    ...
    describe('st-extends', () => {
        it('should add inherit check rule in dev mode', () => {
            ...
            const devActual = devEntry.outputAst!.toString().replace(/\s\s+/g, ' ');
            const prodActual = prodEntry.outputAst?.toString().replace(/\s\s+/g, ' ');
        ...
        it('should not add inherit check rule for mixin', () => {
            ...
            expect((meta.outputAst!.nodes[0] as postcss.Rule).selector).to.equal('.entry__root');
        ...
        it('should use -st-global in inherit check', () => {
            ...
            const actual = meta.outputAst!.toString().replace(/\s\s+/g, ' ');
            const expected = CSSClass.createWarningRule(
                '.root',
                '.y',
                'x.st.css',
