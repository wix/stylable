...
describe('Inner Directives', () => {
    ...
    it('should not complete -st-from, -st-default, -st-named outisde the import ruleset', () => {
        const asserter = asserters.getCompletions('imports/outside-ruleset.st.css');
        asserter.notSuggested([
            asserters.importFromDirectiveCompletion(createRange(0, 0, 0, 0)),
            asserters.importDefaultDirectiveCompletion(createRange(0, 0, 0, 0)),
            asserters.importNamedDirectiveCompletion(createRange(0, 0, 0, 0))
    ...
    it('should complete -st-mixin, but not -st-states, -st-extends inside media query', () => {
        const asserter = asserters.getCompletions('complex-selectors/media-query.st.css');
