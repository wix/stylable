...
describe('Variables Directive', () => {
    ...
    it('should not complete :vars after ::', () => {
        const asserter = asserters.getCompletions('imports/top-level-colon-colon.st.css');
        asserter.notSuggested([asserters.varsDirectiveCompletion(createRange(0, 0, 0, 0))]);
    });

    it('should not complete :vars inside rulesets', () => {
        const asserter = asserters.getCompletions('imports/inside-ruleset.st.css');
        asserter.suggested([]);
        asserter.notSuggested([asserters.varsDirectiveCompletion(createRange(0, 0, 0, 0))]);
    });

    it('should not complete :vars inside media query', () => {
