...
export function resolveNamespaceFactory(hashSalt: string = '', prefix: string = ''): typeof processNamespace {
    return (namespace: string, stylesheetPath: string) => {
        ...
        return prefix + namespace + hash.v3(hashSalt + config.name + '@' + config.version + '/' + fromRoot);
    };
}
