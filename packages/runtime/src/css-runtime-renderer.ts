...
export class RuntimeRenderer {
    ...
    public setStyles(ids: string[], sortIndexes: boolean) {
        this.styles = this.getStyles(ids, sortIndexes);
        this.update();
    }
    public getStyles(ids: string[], sortIndexes: boolean) {
        // const shallow = ids.map(id => this.stylesMap[id]);
        return this.sortStyles(this.expandDependencies(ids), sortIndexes);
    }
    public expandDependencies(styleEntries: string[]) {
        const cache: { [key: string]: boolean } = {};
        const fullList = [];
        const entries = styleEntries.slice();
        while (entries.length) {
            const styleID = entries.pop()!;
            const style = this.stylesMap[styleID];
            if (style && !cache[styleID]) {
                cache[styleID] = true;
                fullList.push(style);
                if (style.$cssDeps) {
                    entries.push(...style.$cssDeps);
                }
            }
        }
        return fullList;
