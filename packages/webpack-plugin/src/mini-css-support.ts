...
import { StylablePlugin } from "./plugin";
...
export function injectCssModules(
  ...
  compilation.hooks.afterChunks.tap(StylablePlugin.name, () => {
    for (const module of stylableModules) {
      const cssModule = new CssModule({
        context: module.context,
        identifier:
          module.resource.replace(".st.css", ".css") + "?stylable-css-inject",
        identifierIndex: 1,
...
export function injectCSSOptimizationRules(compiler: Compiler) {
  ...
  if (
    compiler.options.optimization.splitChunks.cacheGroups.stylable === undefined
  ) {
