...
export function extractFilenameFromAssetModule(m: Module): string {
  ...
  const match = source
    .toString()
    .match(/__webpack_public_path__\s*\+\s*"(.*?)"/);
  if (match) {
    return match[1];
  }
...
export function getAssetOutputPath(
  ...
  const assetModuleFilename =
    staticFilename || runtimeTemplate.outputOptions.assetModuleFilename;
...
export function createStaticCSS(
  ...
  const cssChunks = Array.from(stylableModules)
    ...
    .sort(
      (m1, m2) =>
        m1.buildMeta.stylable.cssDepth - m2.buildMeta.stylable.cssDepth
    )
    .map((m) =>
      replaceCSSAssetPlaceholders(
        ...
        staticPublicPath,
        (resourcePath) => {
          let assetModule = assetsModules.get(resourcePath);
          if (!assetModule) {
